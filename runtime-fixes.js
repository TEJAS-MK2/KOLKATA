...
function boot(){window.KOLKATA_NORMALIZE_PANDALS?.(window.pandals);syncMode();exposeSelector();patchCatalogCards();patchCompanionDates();fixGalleryA11y();normalizeLinks();fixLeafletAssetUrls();registerSW();setTimeout(()=>{patchCatalogCards();patchCompanionDates();fixGalleryA11y();normalizeLinks();fixLeafletAssetUrls()},1200);setTimeout(()=>{patchCatalogCards();patchCompanionDates();normalizeLinks();fixLeafletAssetUrls()},3200)}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot,{once:true});else boot();
window.addEventListener('load',()=>{patchCatalogCards();normalizeLinks();fixLeafletAssetUrls()},{once:true});
})();
