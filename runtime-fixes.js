...
function boot(){window.KOLKATA_NORMALIZE_PANDALS?.(window.pandals);syncMode();exposeSelector();patchCatalogCards();patchCompanionDates();fixGalleryA11y();normalizeLinks();registerSW();setTimeout(()=>{patchCatalogCards();patchCompanionDates();fixGalleryA11y();normalizeLinks()},1200);setTimeout(()=>{patchCatalogCards();patchCompanionDates();normalizeLinks()},3200)}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot,{once:true});else boot();
window.addEventListener('load',()=>{patchCatalogCards();normalizeLinks()},{once:true});
})();
