(() => {
  ...
  function restore() {
    try { const saved=JSON.parse(localStorage.getItem(ROUTE_KEY)||'[]'); route=saved.map(x=>data().find(p=>p.name===x.name)).filter(Boolean).slice(0,MAX_STOPS); } catch { route=[]; }
    try { const saved=JSON.parse(localStorage.getItem(DONE_KEY)||'[]'); done=new Set(Array.isArray(saved)?saved:[]); } catch { done=new Set(); }
  }
  function persist(){try{localStorage.setItem(ROUTE_KEY,JSON.stringify(route.map(p=>({name:p.name}))));localStorage.setItem(DONE_KEY,JSON.stringify([...done]));}catch{}}
  ...
  function select(p){
    if(!p)return;
    if(map&&valid(p)){map.flyTo([Number(p.lat),Number(p.lng)],15,{duration:.5});markers.find(x=>x.p.name===p.name)?.marker.openPopup();}
  ...
  function renderRoute(){
    ...
    document.querySelectorAll('.add-route').forEach(b=>{const p=data().find(x=>x.name===b.dataset.name),added=inRoute(p);b.textContent=added?'Added ✓':route.length>=MAX_STOPS?'Route full':'Add to route +';b.disabled=added||route.length>=MAX_STOPS;});
    renderNight();
  }
  ...
  function renderNight(){
    const total=route.length,p=route[nightIndex];const label=document.querySelector('#night-stop-label'),name=document.querySelector('#night-stop-name'),area=document.querySelector('#night-stop-area'),bar=document.querySelector('#night-progress-bar'),text=document.querySelector('#night-progress-text'),prev=document.querySelector('#night-prev'),next=document.querySelector('#night-next'),dir=document.querySelector('#night-directions');
