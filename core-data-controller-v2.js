(() => {
  ...
  const currentState = () => window.KolkataState?.get?.() || {route:[],completed:[],mode:'walking'};
  const mode = () => { const value=currentState().mode; return ['walking','driving','transit','two-wheeler'].includes(value) ? (value==='two-wheeler'?'driving':value) : 'walking'; };
  ...
  function restore() {
    ...
    route=(Array.isArray(saved.route)?saved.route:[]).map(x=>data().find(p=>p.name===x.name)).filter(Boolean).slice(0,MAX_STOPS);
    done=new Set(Array.isArray(saved.completed)?saved.completed:[]);
  }
  function persist(){window.KolkataState?.update?.({route:route.map(p=>({name:p.name,lat:Number(p.lat),lng:Number(p.lng)})),completed:[...done]});}
  ...
  function select(p){if(!p)return;if(map&&valid(p)){map.flyTo([Number(p.lat),Number(p.lng)],15,{duration:.5});markers.find(x=>x.p.name===p.name)?.marker.openPopup()}document.querySelectorAll('.pandal-card').forEach(c=>c.classList.toggle('selected',c.dataset.name===p.name));}
  ...
  function renderExplorer(){
    ...
    const active=document.querySelector('.filter-btn.active')?.dataset.zone||'all';
    const items=data().filter(p=>(active==='all'||!active||p.zone===active)&&(!query||`${p.name} ${p.area} ${p.zone} ${p.tag}`.toLowerCase().includes(query)));
    ...
    list.querySelectorAll('.pandal-card').forEach(card=>{const p=data().find(x=>x.name===card.dataset.name);card.addEventListener('click',e=>{if(!e.target.closest('a,button'))select(p)});card.addEventListener('keydown',e=>{if((e.key==='Enter'||e.key===' ')&&!e.target.closest('button')){e.preventDefault();select(p)}})});
  ...
  function rebuildMap(){
    const host=document.querySelector('#pandal-map');if(!host||!window.L)return;
    if(window.kolkataMap){try{window.kolkataMap.remove()}catch{}}
    map=window.L.map(host,{scrollWheelZoom:false}).setView([22.5726,88.3639],12);window.kolkataMap=map;window.map=map;
    ...
    markers=data().filter(valid).map(p=>{const marker=window.L.marker([Number(p.lat),Number(p.lng)],{title:p.name,alt:p.name}).addTo(map);marker.bindPopup(`<strong>${esc(p.name)}</strong><br>${esc(p.area)} · ${esc(p.zone)}<br><button type="button" class="popup-add" data-pandal="${esc(p.name)}">Add to route</button> · <a href="${maps(p)}" target="_blank" rel="noopener">Directions ↗</a>`);marker.on('popupopen',()=>document.querySelector(`.popup-add[data-pandal="${CSS.escape(p.name)}"]`)?.addEventListener('click',()=>add(p),{once:true}));return {p,marker}});
  }
  function routeUrl(){if(!route.length)return 'https://www.google.com/maps/dir/?api=1';const stops=route.slice(0,MAX_STOPS),last=stops[stops.length-1];let u=`https://www.google.com/maps/dir/?api=1&destination=${encodeURIComponent(`${last.lat},${last.lng}`)}&travelmode=${encodeURIComponent(mode())}`;const wp=stops.slice(0,-1).map(p=>`${p.lat},${p.lng}`).join('|');if(wp)u+=`&waypoints=${encodeURIComponent(wp)}`;return u;}
  function renderRoute(){const count=document.querySelector('#route-count');if(count)count.textContent=`${route.length} / ${MAX_STOPS}`;const wrap=document.querySelector('#route-stops');if(wrap)wrap.innerHTML=route.length?route.map((p,i)=>`<div class="route-stop"><span class="route-stop-index">${i+1}</span><div><strong>${esc(p.name)}</strong><small>${esc(p.area)} · ${esc(p.zone)}</small></div><button class="remove-stop" type="button" data-remove="${esc(p.name)}" aria-label="Remove ${esc(p.name)}">×</button></div>`).join(''):'<div class="empty-state"><strong>Your route is empty.</strong><p>Tap “Add to route” on pandals above.</p></div>';wrap?.querySelectorAll('[data-remove]').forEach(b=>b.addEventListener('click',()=>remove(b.dataset.remove)));const link=document.querySelector('#open-route');if(link){link.href=routeUrl();link.setAttribute('aria-disabled',String(!route.length))}document.querySelectorAll('.add-route').forEach(b=>{const p=data().find(x=>x.name===b.dataset.name),added=inRoute(p);b.textContent=added?'Added ✓':route.length>=MAX_STOPS?'Route full':'Add to route +';b.disabled=added||route.length>=MAX_STOPS});renderNight();}
