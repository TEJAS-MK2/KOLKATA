...
const pandals = [
  {name:'Bagbazar Sarbojanin',zone:'North',area:'Bagbazar',lat:22.60121,lng:88.36682,tag:'Heritage favourite',photo:'https://commons.wikimedia.org/wiki/Special:FilePath/%E0%A6%AC%E0%A6%BE%E0%A6%97%E0%A6%AC%E0%A6%AC%E0%A6%BE%E0%A6%9C%E0%A6%BE%E0%A6%B0_%E0%A6%B8%E0%A6%BE%E0%A6%B0%E0%A7%8D%E0%A6%AC%E0%A6%9C%E0%A6%BE%E0%A6%A8%E0%A7%80%E0%A6%A8_%E0%A6%A6%E0%A7%81%E0%A6%B0%E0%A7%8D%E0%A6%97%E0%A7%8B%E0%A7%8E%E0%A6%B8%E0%A6%AC_%E0%A7%A8%E0%A5%A6%E0%A5%A7%E0%A7%AE.jpg',rating:4.8},
...
function renderNightMode(){
  ...
  if(!total){if(label)label.textContent='STOP 1 OF 0';if(name)name.textContent='Add stops to begin';if(area)area.textContent='Your selected route will appear here.';if(bar)bar.style.width='0%';if(text)text.textContent='0 / 0 completed';if(directions)directions.href=routeUrl();if(prev)prev.disabled=true;if(next)next.disabled=true;refreshRouteIntelligence();return;}
...
function haversineKm(a,b){const R=6371,dLat=(b.lat-a.lat)*Math.PI/180,dLon=(b.lng-a.lng)*Math.PI/180;const lat1=a.lat*Math.PI/180,lat2=b.lat*Math.PI/180;const h=Math.sin(dLat/2)**2+Math.cos(lat1)*Math.cos(lat2)*Math.sin(dLon/2)**2;return 2*R*Math.asin(Math.sqrt(h));}
function modeProfile(){return {walking:{speed:4.5,detour:1.18,label:'walking'},driving:{speed:22,detour:1.28,label:'driving'},transit:{speed:17,detour:1.35,label:'transit'},'two-wheeler':{speed:25,detour:1.25,label:'two-wheeler'}}[travelMode]||{speed:4.5,detour:1.18,label:'walking'};}
function formatMinutes(min){if(min<1)return '<1 min';if(min<60)return `${Math.round(min)} min`;const h=Math.floor(min/60),m=Math.round(min%60);return `${h} hr${h>1?'s':''}${m?` ${m} min`:''}`;}
function getRouteMetrics(stops=routeStops){const profile=modeProfile();let km=0,legs=[];for(let i=1;i<stops.length;i++){const direct=haversineKm(stops[i-1],stops[i]);const distance=direct*profile.detour;km+=distance;legs.push({from:stops[i-1],to:stops[i],distance,time:(distance/profile.speed)*60});}return {distance:km,time:legs.reduce((sum,l)=>sum+l.time,0),legs};}
function nearestNeighbourRoute(stops){if(stops.length<3)return [...stops];const remaining=stops.slice(1),ordered=[stops[0]];while(remaining.length){const from=ordered[ordered.length-1];let bestIndex=0,best=Infinity;remaining.forEach((p,i)=>{const d=haversineKm(from,p);if(d<best){best=d;bestIndex=i;}});ordered.push(remaining.splice(bestIndex,1)[0]);}return ordered;}
function refreshRouteIntelligence(){
  const summary=document.querySelector('#route-intelligence');
  if(!summary)return;
  if(routeStops.length<2){summary.innerHTML='<div class="route-intel-empty">Add at least 2 stops to see an estimated route distance and travel time.</div>';return;}
  const metrics=getRouteMetrics();
  const profile=modeProfile();
  summary.innerHTML=`<div class="route-intel-grid"><div><span>EST. DISTANCE</span><strong>${metrics.distance.toFixed(1)} km</strong></div><div><span>EST. ${profile.label.toUpperCase()} TIME</span><strong>${formatMinutes(metrics.time)}</strong></div><div><span>STOPS</span><strong>${routeStops.length}</strong></div></div><div class="route-intel-legs">${metrics.legs.map((leg,i)=>`<div><span>${i+1}</span><p><strong>${leg.from.name}</strong> → <strong>${leg.to.name}</strong><small>≈ ${leg.distance.toFixed(1)} km · ${formatMinutes(leg.time)}</small></p></div>`).join('')}</div>`;
}

document.querySelector('#optimize-route')?.addEventListener('click',()=>{if(routeStops.length<3)return;routeStops=nearestNeighbourRoute(routeStops);nightIndex=0;persistNight();renderRoute();renderPandalExplorer();renderNightMode();});