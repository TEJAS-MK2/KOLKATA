(()=>{
'use strict';
if(window.L&&!window.L.markerClusterGroup){window.L.markerClusterGroup=()=>window.L.layerGroup()}
function ensureMenuStyles(){
  if(document.getElementById('kolkata-menu-core-css'))return;
  const style=document.createElement('style');style.id='kolkata-menu-core-css';
...
function initMenuFallback(){
  ...
  const setOpen=open=>{
    ...
    nav.classList.toggle('is-open',open);header?.classList.toggle('menu-open',open);document.documentElement.classList.toggle('menu-is-open',open);
  };
  setOpen(false);
  menu.addEventListener('click',event=>{event.preventDefault();event.stopImmediatePropagation();setOpen(menu.getAttribute('aria-expanded')!=='true')},true);
  nav.querySelectorAll('a').forEach(link=>link.addEventListener('click',()=>setOpen(false)));
  document.addEventListener('keydown',event=>{if(event.key==='Escape')setOpen(false)});
}
function showFallback(){
  const map=document.querySelector('#pandal-map');if(!map||window.L||map.dataset.fallbackShown)return;
  map.dataset.fallbackShown='true';map.innerHTML='<div class="map-fallback"><strong>Interactive map unavailable</strong><p>The map library could not be loaded. Search and Google Maps directions remain available.</p></div>';
